...
class BaseInputReader(ABC):
    def __init__(self, types_path: str, tokenizer: BertTokenizer, neg_entity_count: int = None,
        ...
        types = json.load(open(types_path), object_pairs_hook=OrderedDict)  # entity + relation types  实体和关系类型

        self._entity_types = OrderedDict()#实体类别
        self._idx2entity_type = OrderedDict()#实体类别索引
        self._relation_types = OrderedDict()#关系类别
        self._idx2relation_type = OrderedDict()#关系类别索引

        # entities
        # add 'None' entity type
        none_entity_type = EntityType('None', 0, 'None', 'No Entity')#实体对象
...
class JsonInputReader(BaseInputReader):
    ...
    def read(self, dataset_path, dataset_label):
        ...
        self._parse_dataset(dataset_path, dataset)#构建最终数据集
        self._datasets[dataset_label] = dataset
        return dataset

    def _parse_dataset(self, dataset_path, dataset):
        documents = json.load(open(dataset_path))#原始数据加载
        for document in tqdm(documents, desc="Parse dataset '%s'" % dataset.label):
            self._parse_document(document, dataset)#解析文档
    ...
    def _parse_entities(self, jentities, doc_tokens, dataset) -> List[Entity]:
        ...
        for entity_idx, jentity in enumerate(jentities):
            ...
            tokens = doc_tokens[start:end]#获取实体对应的token列表
            phrase = " ".join([t.phrase for t in tokens])#实体对应的文本内容
    ...
    def _parse_relations(self, jrelations, entities, dataset) -> List[Relation]:
        ...
        for jrelation in jrelations:
            ...
            head_idx = jrelation['head']#这是主体在实体列表中的索引
            tail_idx = jrelation['tail']#这是客体在实体列表中的索引

            # create relation
            head = entities[head_idx]#获取主体对应的实体对象
            tail = entities[tail_idx]#获取客体对应的实体对象

            reverse = int(tail.tokens[0].index) < int(head.tokens[0].index)#判断实体在句子中的顺序
...
def _parse_tokens(jtokens, dataset, tokenizer):
    ...
    for i, token_phrase in enumerate(jtokens):
        token_encoding = tokenizer.encode(token_phrase, add_special_tokens=False)#转换为id列表
        #这里需要注意，可能将某些词拆分成了子词，如果一个词无法识别，则返回空列表，所以一般doc_tokens的长度小于doc_encoding的长度
